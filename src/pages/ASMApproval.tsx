...
export default function ASMApproval() {
  const [orders, setOrders] = useState<Order[]>(mockOrders);
  const { toast } = useToast();

  const pendingOrders = orders.filter((o) => o.status === "sales_approved");
  ...
  const handleApprove = (orderId: string) => {
    setOrders(
      orders.map((o) =>
        o.id === orderId
          ? { ...o, status: "asm_approved" as OrderStatus, asm: "Sarah Johnson" }
          : o
      )
    );
    toast({
      title: "Order Approved",
      description: "Order has been approved and sent to GM for review.",
    });
  };
  ...
  const handleReject = (orderId: string) => {
    ...
    toast({
      ...
      description: "Order has been rejected. Sales team will be notified.",
      variant: "destructive",
    });
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">ASM Approval</h1>
        <p className="text-muted-foreground mt-1">
          Review sales-approved orders and track approved orders
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium">Pending ASM Review</CardTitle>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Pending ASM Approval</CardTitle>
        </CardHeader>
        <CardContent>
          {pendingOrders.length === 0 ? (
            <p className="text-muted-foreground text-center py-8">
              No orders pending ASM approval
            </p>
          ) : (
            ...
            {myApprovals.slice(0, 8).map((order) => (
              ...
              >
                <div className="space-y-1 flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Sales: {order.salesPerson} • ASM: {order.asm}
                  </p>
                  {order.gm && (
                    <p className="text-sm text-muted-foreground">
                      GM: {order.gm}
